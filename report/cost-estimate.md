...
**Estimate date:** YYYY-MM-DD (fill in)
...
1. App Service Plan autoscale rules (CPU > 70% scale-out, CPU < 30% scale-in, min 2 max 4 instances).
...
2. Add: App Service > Standard S1, Linux, 2 instances, region Southeast Asia.
3. Add: SQL Database > Single Database, DTU model, S0, region Southeast Asia.
...
|---|---|---|
| Use Free F1 App Service tier for the demo | Swap App Service Plan SKU from S1 to F1 (no autoscale, single instance, 1 GB RAM). | ~$146/mo (App Service cost drops to $0). |
...
**Chosen recommendation for this project:** keep the S1 plan during grading week to demonstrate autoscale; switch to F1 for the rest of the development period, and delete the resource group immediately after the live demo. Estimated savings vs. running S1 for a full month: ~$130 USD.
